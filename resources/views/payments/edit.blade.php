...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active"><a href="#">Edit {{$payment->name}} payment</a></li>
        </ol>
    </section>
                    ...
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group {{$errors->has('expense_item_id') ? 'has-error' : ''}}">
                                ...
                                <select name="expense_item_id" id="expense_item_id" class="form-control">
                                    ...
                                    @foreach($items as $item)
                                        <option {{$payment->expense_item_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group {{$errors->has('notes') ? 'has-error' : ''}}">
                                ...
                                <textarea placeholder="(Optional)" name="notes" id="" rows="6" class="form-control">{{$payment->notes}}</textarea>
                                @if($errors->has('notes'))
                            ...
                            <div class="form-group {{$errors->has('actual_expense') ? 'has-error' : ''}}">
                                ...
                                <input name="actual_expense" class="form-control" id="expense" value="{{$payment->actual_expense}}">
                                <input type="hidden" name="expected_expense" class="form-control" id="expected_expense" value="{{$payment->expected_expense}}">
                            ...
                            <div class="form-group {{$errors->has('expense_date') ? 'has-error' : ''}}">
                                ...
                                <div class='input-group date' id='datetimepicker1'>
                                    <input type="text" class="form-control" name="expense_date" value="{{$payment->date}}" placeholder="Y-m-d"
                                           id="expense_date"/>
